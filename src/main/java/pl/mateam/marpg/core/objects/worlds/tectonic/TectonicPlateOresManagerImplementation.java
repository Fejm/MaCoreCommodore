...
public class TectonicPlateOresManagerImplementation extends AbstractTectonicPlateManager<TectonicPlateOresManager> implements TectonicPlateOresManager {
	public TectonicPlateOresManagerImplementation(TectonicPlate parentPlate) {
		super(parentPlate, TectonicPlate::getOresManager);
	}
	...
	@Override public int[] getOreChances(boolean relative)	{
		if(relative || parent == null)
			...
			for(int i = 0; i < result.length; i++)
				result[i] = ((TectonicPlateOresManagerImplementation) parent).oresAppearChance[i] * oresAppearChance[i];
			return result;
		}
	}
	...
	@Override public void replenishOre(Block block, boolean playEffect, boolean updateSubplates) {
		...
		if(task != null) {
			...
			if(isRelative) {
				int[] parentValues = parent == null? coreOresAppearChance : ((TectonicPlateOresManagerImplementation) parent).oresAppearChance;
				int sum = 0;
